...
pub use simple::{
    ...
    UserSystem,
    create_user_system,
    create_user_system_with_demo_keys,
    SimpleUserBuilder,
    SimpleBlockBuilder,
    SimpleExecutableBlockBuilder,
    SimpleUserSystemFactory,
};
...
impl From<UserSystemError> for UserError {
    fn from(error: UserSystemError) -> Self {
        match error {
            ...
            UserSystemError::BlockNotFound => UserError::InvalidOperation("Block not found".to_string()),
            UserSystemError::BlockAlreadyExists => UserError::InvalidOperation("Block already exists".to_string()),
        }
    }
}
...
impl UserAuthenticatorCompat {
    ...
    pub fn authenticate(
        ...
        let user = self.user_system.get_user(user_id)
    ...
    pub fn validate_token(&self, token: &AuthTokenCompat) -> Result<User, UserError> {
        ...
        let user = self.user_system.get_user(&token.user_id)
            .ok_or(UserError::UserNotFound)?;
    ...
    pub fn get_genesis_user(&self) -> Result<User, UserError> {
        self.user_system.get_genesis_user()
            .ok_or(UserError::UserNotFound)
    }
}
...
mod tests {
    ...
    fn test_system_block_ownership() {
        ...
        let kernel_block_id = system.register_system_executable(
            "kernel".to_string(),
            kernel_code.clone(),
            "main".to_string(),
            "kernel".to_string(),
